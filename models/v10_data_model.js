...
const getV10Data = async () => {
  ...
  for (let i = 0; i < SET_ID.length; i++) {
    ...
    const result = await db.query(
      "select * from datasets where set_id=$1 order by measurement_date::int asc",
      [ID]
    );
  ...
  for (let i = 0; i < SET_ID.length; i++) {
    ...
    if (i == 0) {
      for (let j = 0; j < V10V4Data.data.length; j++) {
        ...
        await db.query(
          "insert into datasets (set_id, measurement_date, data, string) values ($1,$2,$3,$4)",
          [ID, element.Year, 0, element.Event]
        );
      }
    } else {
